...
def getPeopleInfo(request):
    data=json.loads(request.body) #요청 바디에서 json 형식을 딕셔너리 자료형으로 변환
...
def update_showrange(request):
    ...
    if user_id is None or protector_id is None or new_range is None:
        return JsonResponse({"message": "user_id, protector_id 또는 공개범위가 누락되었습니다."}, status=400)

    try:
        owner = People.objects.get(id=user_id)
        protector = People.objects.get(id=protector_id)
    except People.DoesNotExist:
        return JsonResponse({"message": "사용자 또는 보호자를 찾을 수 없습니다."}, status=404)
    ...
    if sharing.share_state != 'matched':
        return JsonResponse({"message": "아직 연동이 완료되지 않은 보호자입니다."}, status=403)

    VALID_RANGE = ['private', 'partial', 'full']
    if new_range not in VALID_RANGE:
        return JsonResponse({"message": f"공개범위 값은 {VALID_RANGE} 중 하나여야 합니다."}, status=400)

    sharing.share_range = new_range
    sharing.save()

    return JsonResponse({"message": "공개범위가 성공적으로 수정되었습니다."}, status=200)
...
def search_user_by_id(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return JsonResponse({'exists': True, 'name': user.name, 'found_id': user.string_id})
        except People.DoesNotExist:
...
def handle_sharing_request(request):
    ...
    try:
        ...
        return JsonResponse({'error': '요청 대상 사용자가 존재하지 않습니다.'}, status=404)
