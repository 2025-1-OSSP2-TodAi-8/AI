...
def search_user_by_id(request):
    string_id = request.data.get("string_id")

    if not string_id:
        return Response(
            {"message": "string_id가 누락되었습니다."},
...
def handle_sharing_request(request):
    ...
    target_string_id = data.get("target_string_id")  # 공유 요청 대상 유저
    relation = data.get("relation")

    if not target_string_id or not relation:
        return Response(
            {"message": "target_string_id 또는 relation이 누락되었습니다."},
            status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if Sharing.objects.filter(owner=owner, shared_with=requester).exists():
        return Response(
    ...
    Sharing.objects.create(owner=owner, shared_with=requester, relation=relation)
